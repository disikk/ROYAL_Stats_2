...
- **Плагинная система парсеров** через `BaseParserPlugin` и `plugin_manager`
  - Встроенные парсеры `HandHistoryParser` и `TournamentSummaryParser`
  - Поддержка внешних плагинов через entry points группы `royal_stats`
...
1. Создайте класс в директории `stats/`, наследующий `BaseStat`, **или** установите
   сторонний пакет c entry point `royal_stats.plugins`.
2. Реализуйте метод `compute()` с единой сигнатурой.
3. Плагин должен работать с сырыми данными (`tournaments`, `hands`, `sessions`).
4. `StatisticsService` автоматически загрузит все плагины из директории `stats`
   и объявленные через entry points. При желании можно передать собственный
   список плагинов в конструктор сервиса.
5. Обновите ViewModel, если требуется отображать новую статистику.
...
1. Наследоваться от `BaseParserPlugin[T]` (или `BaseParser[T]` для встроенных классов)
2. Указать тип обрабатываемого файла в атрибуте `file_type`
3. Реализовать метод `parse()` возвращающий типизированный результат
4. Зарегистрировать плагин через entry point `royal_stats` или передать класс в `ImportService`
5. Обеспечить совместимость с существующими форматами
...
- Используйте параметр `debug` из `AppConfig` (настраивается в `config.ini`) для подробного логирования
