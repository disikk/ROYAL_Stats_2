...
class StatisticsService:
    ...
    def _clear_cache_for_db(self, db_path_to_clear: str) -> None:
        """Removes cache entries associated with a specific database path."""
        logger.info(f"Clearing cache for DB path: {db_path_to_clear}")
        cache_changed = False
        if db_path_to_clear in self._overall_stats_cache:
            del self._overall_stats_cache[db_path_to_clear]
            logger.debug(f"Removed from _overall_stats_cache: {db_path_to_clear}")
            cache_changed = True

        if db_path_to_clear in self._place_distribution_cache:
            del self._place_distribution_cache[db_path_to_clear]
            logger.debug(f"Removed from _place_distribution_cache: {db_path_to_clear}")
            cache_changed = True

        if db_path_to_clear in self._persistent_cache:
            del self._persistent_cache[db_path_to_clear]
            logger.debug(f"Removed from _persistent_cache: {db_path_to_clear}")
            cache_changed = True
