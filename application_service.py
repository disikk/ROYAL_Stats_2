...
class ApplicationService:
    ...
    def __init__(self):
        ...
        self._tournament_repo = TournamentRepository(db_manager=self.db)
        self.session_repo = SessionRepository(db_manager=self.db)
        self.overall_stats_repo = OverallStatsRepository(db_manager=self.db)
        self.place_dist_repo = PlaceDistributionRepository(db_manager=self.db)
        self.ft_hand_repo = FinalTableHandRepository(db_manager=self.db)
    ...
    def import_files(
        ...
        for path in paths:
            ...
            if os.path.isdir(path):
                for _, _, filenames in os.walk(path):
                    if is_canceled_callback and is_canceled_callback():
                        logger.debug(
                            "Импорт отменен пользователем при подсчете файлов."
                        )
                        if progress_callback:
                            progress_callback(0, 0, "Импорт отменен пользователем")
                        return
            ...
            if os.path.isdir(path):
                for root, _, filenames in os.walk(path):
                    for fname in filenames:
                        if is_canceled_callback and is_canceled_callback():
                            logger.debug(
                                "Импорт отменен пользователем при подготовке файлов."
                            )
                            if progress_callback:
                                progress_callback(processed_candidates, total_candidates, "Импорт отменен пользователем")
                            return
        ...
        if all_final_table_hands_data:
            logger.debug(f"Пример первой руки: {all_final_table_hands_data[0]}")
        ...
        logger.debug(
            f"Сохранение рук завершено: {hands_saved} из {total_hands_to_save}"
        )
        ...
        if hands_saved > 0:
            try:
                conn = self.db.get_connection()
                conn.commit()
                logger.debug(f"Commit выполнен для {hands_saved} рук")
            except Exception as e:
        ...
        logger.debug("=== ПРОВЕРКА БД ПОСЛЕ ИМПОРТА ===")
    ...
    def _calculate_overall_stats(self) -> OverallStats:
        ...
        big_ko_results = BigKOStat().compute(all_tournaments, all_ft_hands, [])
